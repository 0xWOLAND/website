...
PLONKish systems have matured into an entire developer stack: there's a full suite of DSLs, circuit libraries, recursive provers, and zkVMs that make them stable and production-grade. High-level languages like Circom by Polygon co-founder Jordi Baylina, Noir, and Leo compile directly into PLONK circuits. There's also `gnark`, `jellyfish`, and countless other repos that provide ready-made gadgets for common primitives like Merkle trees, hashes, etc. Aside from Halo2, recursive provers like Plonky2 are actively deployed in scalable zkRollups, and their reach is being amplified by zkVMs[^2] such as [Succinct's SP1](http://succinct.xyz/) and [RISC Zero](https://risczero.com/).
...
But the way that Monero is pursuing this upgrade is anything but simple. FCMP++'s curve tree is an incredibly delicate construction: multiple composed heterogeneous Bulletproofs, divisor arithmetic across different elliptic curves, custom linkability and spend authorization gadgets, etc. And each component must be proven sound and implemented correctly in a monolithic Bulletproof, yet another daunting task. 
